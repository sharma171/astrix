...
const Hero = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const heroElement = document.querySelector('.hero-anim');
      if (heroElement) {
        const rect = heroElement.getBoundingClientRect();
        const bottomOfHeroVisible = rect.bottom <= (window.innerHeight || document.documentElement.clientHeight);
        if (bottomOfHeroVisible) {
          setIsActive(true);
        } else {
          setIsActive(false);
        }
    ...
    return (
        <div className="floating-hero">
            ...
            <div className={`hero-anim row-flex ${isActive ? 'active' : ''}`}>
